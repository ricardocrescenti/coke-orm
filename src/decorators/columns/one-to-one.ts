...
/**
 * Decorator for 1-1 relationship entity columns
 * @param {ColumnOptions} options Column options
 * @return {Function} Returns the function needed by the Decorator
 */
export function OneToOne<T>(options?: Omit<ColumnOptions<T, Omit<ForeignKeyOptions<T>, 'target' | 'type'>>, 'target' | 'propertyName' | 'propertyType' | 'enum' | 'operation'>) {
