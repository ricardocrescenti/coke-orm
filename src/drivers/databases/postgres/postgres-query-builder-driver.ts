...
export class PostgresQueryBuilderDriver extends QueryBuilderDriver {
	...
	public createTriggerFromMetadata(triggerMetadata: TriggerMetadata): string[] {
		...
		return [
			...
			`CREATE ${triggerMetadata.trigger.constraintTrigger ? 'CONSTRAINT ' : ''}TRIGGER "${triggerMetadata.name}" ${triggerMetadata.trigger.fires} ${triggerMetadata.trigger.events.join(' OR ')} ON "${triggerMetadata.entity.connection.options.schema ?? 'public'}"."${triggerMetadata.entity.name}" ${triggerMetadata.trigger.deferrable ? 'DEFERRABLE ' : ''}${triggerMetadata.trigger.deferred ? 'INITIALLY DEFERRED ' : ''}FOR EACH ROW ${triggerMetadata.trigger.when ? ` WHEN ${triggerMetadata.trigger.when} ` : ''} EXECUTE FUNCTION "${triggerMetadata.entity.connection.options.schema ?? 'public'}"."${triggerMetadata.name}"();`,
